...
class TermsController extends Controller
{
    ...
    protected $controller = 'terms';
    protected $folder = 'terms';
    protected $menu_id = '38';
    protected $name_page = "Terms And Condition";
    ...
    public function datatable(Request $request){
        ...
        $sTable = TermsModel::orderby('id','desc')
            ->when($like, function ($query) use ($like) {
                if (@$like['search_title'] != "") {
                    $query->where('title', 'like', '%' . $like['search_title'] . '%');
                }
            });
        ...
        ->addColumn('action', function ($row) {
            ...
            // return "                                        
            //     <a href='$this->segment/$this->folder/edit/$row->id' class='mr-3 mb-2 btn btn-sm btn-info' title='Edit'><i class='fa fa-edit w-4 h-4 mr-1'></i> แก้ไข </a>                                                 
    ...
    {
        $query = TermsModel::destroy($request->id);
        if ($query) {
            return response()->json(true);
        } else {
            return response()->json(false);
