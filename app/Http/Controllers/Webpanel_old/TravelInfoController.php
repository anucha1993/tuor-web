...
class TravelInfoController extends Controller
{
    ...
    protected $controller = 'travel';
    protected $folder = 'travel';
    protected $menu_id = '4';
    protected $name_page = "รอบรู้เรื่องเที่ยว";
    ...
    public function index(Request $request)
    {
        ...
        $navs = [
            '1' => ['url' => "$this->segment/$this->folder", 'name' => "รอบรู้เรื่องเที่ยว", "last" => 1],
        ];
        $country = CountryModel::where('status','on')->whereNull('deleted_at')->get();
        $type  = TypeArticleModel::get();
    ...
    {
        $like = $request->Like;
        $sTable = TravelModel::orderby('id', 'desc')
        ->when($like, function ($query) use ($like) {
            ...
            if(@$like['type_article'] != ""){
                $query->where('type_id', 'like', '%' . $like['type_article'] . '%');
            }
        })
        ->get();
        return Datatables::of($sTable)
            ...
            ->addColumn('img', function ($row) {
                ...
                $data = "<center> <img src='$row->img_cover' style='width: 30%;'></center>";
                return $data;
            })
            ...
            ->addColumn('type', function ($row) {
                $type = TypeArticleModel::find($row->type_id);
                $data = $type->type;
                return $data;
            })
            ->rawColumns(['created_at', 'action','status','country','img','type'])
            ->make(true);
    }
    ...
    public function store($request, $id = null)
    {
        try {
            ...
            if ($image) {
                ...
                $ext = explode("/", $lg->mime());
                $lg->resize(736, 407)->stream();
                
                $new = 'upload/travel/image' . date('dmY-Hism') . '.' . $ext[1];
    ...
    {
        $datas = TravelModel::find($id);
        if (@$datas) {
            $query = TravelModel::destroy($datas->id);
        }
