...
class LandmassController extends Controller
{
    protected $prefix = 'backend';
    protected $segment = 'webpanel';
    protected $controller = 'landmass';
    protected $folder = 'landmass';
    protected $menu_id = '24';
    protected $name_page = "ข้อมูลทวีป";
    ...
    public function datatable(Request $request){
        ...
        $sTable = LandmassModel::orderby('id','desc')
            ->when($like, function ($query) use ($like) {
                if (@$like['search_name'] != "") {
                    $query->where('landmass_name', 'like', '%' . $like['search_name'] . '%');
                }
    ...
    public function store($request, $id = null)
    {
        try {
            DB::beginTransaction();
            if ($id == null) {
                $data = new LandmassModel();
                $data->created_at = date('Y-m-d H:i:s');
                $data->updated_at = date('Y-m-d H:i:s');
                if($request->name){
                    $check = LandmassModel::where(['landmass_name'=>$request->name])->first();
                    if($check){
                ...
                $data = LandmassModel::find($id);
                $data->updated_at = date('Y-m-d H:i:s');
            }
            $data->landmass_name = $request->name;
            if ($data->save()) {
                DB::commit();
    ...
    {
        $datas = LandmassModel::find($request->id);
        if (@$datas) {
