...
class TypeArticleController extends Controller
{
    ...
    protected $controller = 'type-article';
    protected $folder = 'type-article';
    protected $menu_id = '45';
    protected $name_page = "ประเภทบทความ";
    ...
    public function index(Request $request)
    {
        ...
        $navs = [
            '0' => ['url' => "$this->segment/$this->folder", 'name' => "คอนเทนต์", "last" => 0],
            '1' => ['url' => "$this->segment/$this->folder", 'name' => "ประเภทบทความ", "last" => 1],
        ];
    ...
    {
        $like = $request->Like;
        $sTable = TypeArticleModel::orderBy('id','desc')
        ->when($like, function ($query) use ($like) {
            if (@$like['name'] != "") {
                $query->where('type', 'like', '%' . $like['name'] . '%');
            }
        })
        ->get();
    ...
    {
        $datas = TypeArticleModel::find($id);
        if (@$datas) {
            $query = TypeArticleModel::destroy($datas->id);
        }
