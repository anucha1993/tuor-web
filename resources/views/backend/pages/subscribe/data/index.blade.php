    ...
    <div class="flex">
            ...
            <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                <div class="w-full sm:w-auto mt-3 mr-2 sm:mt-0 sm:ml-auto md:ml-0">
                    <div class="w-56 relative text-slate-500">
                        <input type="text" class="form-control w-56 box pr-10 myLike" name="search_email" placeholder="ค้นหาอีเมล">
                        <i class="w-4 h-4 absolute my-auto inset-y-0 mr-3 right-0" data-lucide="search"></i>
                    </div>
                </div>
                <div class="hidden md:block mx-auto text-slate-500"></div>
                <div class="float-right">
                    <a class="btn btn-primary btn-md mt-1 btn-excel" href="javascript: void(0);">Export Excel</a>
                    <a class="btn btn-info btn-md mt-1 btn-csv" href="javascript: void(0);">Export CSV</a>
        ...
        <script>
            ...
            $(function () {
                oTable = $('#datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {  data: 'email',      title: '<center>อีเมล</center>',     className: 'items-center w-50 text-center'},
                        {  data: 'created_at',      title: '<center>วันที่สร้าง</center>',     className: 'items-center w20 text-center'},
            ...
            function destroy(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: '{{ url("/webpanel/subscribe/data/delete") }}',
                            data: {
                                id:id,
                                _token:"{{ csrf_token() }}"
                            },
            ...
            $('.btn-excel').on('click', function(e){
                if (confirm('ยืนยันการออกรายงาน อาจใช้เวลาสักครู่ในการสร้าง\nโปรดรอประมาณ 20-60 วินาที ขึ้นอยู่กับจำนวนรายการที่นำมาสร้าง')) {
                    oData.excel=true;
                    $.ajax({
                        type: "POST",
                        dataType: "json",
                        url: '{{ url("webpanel/subscribe/data/datatable") }}',
                        data: oData,
                        success: function( data ) {
                            if (data.redirect) {
