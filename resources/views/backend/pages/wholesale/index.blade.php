            ...
            <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                <div class="w-full sm:w-auto mt-3 mr-2 sm:mt-0 sm:ml-auto md:ml-0">
                    <div class="w-56 relative text-slate-500">
                        <input type="text" class="form-control w-56 box pr-10 myLike" name="search_code" placeholder="ค้นหารหัส">
                        <i class="w-4 h-4 absolute my-auto inset-y-0 mr-3 right-0" data-lucide="search"></i>
                    </div>
                </div>
                <div class="w-full sm:w-auto mt-3 mr-2 sm:mt-0 sm:ml-auto md:ml-0">
                    <div class="w-56 relative text-slate-500">
                        <input type="text" class="form-control w-56 box pr-10 myLike" name="search_name" placeholder="ค้นหาชื่อ Supplier">
                        <i class="w-4 h-4 absolute my-auto inset-y-0 mr-3 right-0" data-lucide="search"></i>
                    </div>
                </div>
                <div class="w-full sm:w-auto mt-3 mr-2 sm:mt-0 sm:ml-auto md:ml-0">
                    <div class="w-56 relative text-slate-500">
                        <select name="search_status" class="form-control w-56 box pr-10 myLike">
            ...
            $(function () {
                oTable = $('#datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {  data: 'code',                title: '<center>รหัส</center>',     className: 'items-center w-20 text-center'},
                        {  data: 'wholesale_name_en',   title: '<center>ชื่อ Supplier</center>',     className: 'items-center w-40 text-center'},
                        {  data: 'tel',                 title: '<center>เบอร์โทรศัพท์</center>',     className: 'items-center w-20 text-center'},
                        {  data: 'contact_person',      title: '<center>ข้อมูลติดต่อ</center>',     className: 'items-center w-20 text-center'},
            ...
            function destroy(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: '{{ url("/webpanel/wholesale/delete") }}',
                            data: {
                                id:id,
                                _token:"{{ csrf_token() }}"
                            },
