...
<body class="py-5">
    ...
    <div class="flex">
                    ...
                    <div class="col-span-12 lg:col-span-6 2xl:col-span-12">
                        <div class="intro-y col-span-12 lg:col-span-6">
                            ...
                            <div class="intro-y box p-5 place-content-center">
                                ...
                                <div class="grid grid-cols-12 gap-6 mt-3 mb-3">
                                    <div class="col-span-12 lg:col-span-12">
                                        <label for="crud-form-1" class="form-label">หัวข้อย่อยและรายละเอียด <button class="btn btn-primary" id="add-row" type="button"style="margin: auto;" colspan="2" onclick="add_column()"><i class="fa fa-plus-circle" ></i>เพิ่มแถว</button></label>
                                        <table class="table table-striped table-bordered" >
                                            <thead class="table-light">
                                                <tr role="row">
                                                    <th  class="vertid text-center sorting_asc" style="width: 20%">หัวข้อย่อย</th>
                                                    <th  class="vertid text-center" style="width: 30%">รายละเอียด</th>
                                                    <th  class="vertid text-center" style="width: 5%">จัดการ</th>
                                                </tr>
                                            </thead>
                                            <tbody id="row_data">
                                                @foreach ($detail as $f => $fea)
                                                    <?php  $num_row = count($detail);    ?>
                                                    @foreach($fea as $d => $de)
                                                    <tr id="del{{$f+1}}">
                                                        <td>
                                                            <input class="form-control" id="topic1" name="detail_topic[]" type="text" value="{{$d}}" >
                                                        </td>
                                                        <td>
                                                            <textarea class="form-control" id="detail1" name="detail[]" rows="3">{{$de}}</textarea>
                                                        </td>
                                                        <td>
                                                            <button class="btn btn-danger del-btn btn-block" type="button" style="margin:auto;" onclick="del_row({{$f+1}})">ลบแถว</button>
                                                        </td>
                                                        <input type="hidden" id="count_row" value="{{$num_row}}">
                                                    </tr>
                                                    @endforeach
                                                @endforeach
                                            </tbody>
        ...
        <script>
        var count_row = document.getElementById('count_row').value*1+1;
        function add_column(){  
